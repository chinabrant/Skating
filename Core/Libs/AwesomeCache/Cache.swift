...
public class Cache<T: NSCoding> {
    ...
    // add by wusj   同步设置文件缓存
    public func setObjectSync(object: T, forKey key: String) {
        let expiryDate = expiryDateForCacheExpiry(.Never)
...
//        var path = cacheDirectory.stringByAppendingPathComponent(key).stringByAppendingPathExtension("cache")!
//        println("path \(path)")
